...
function createMountainInfoHTML(selectedMountain) {
  return `
    <h2>${selectedMountain.name}</h2>
    <p><span style="font-weight: bold">
    Elevation:</span> ${selectedMountain.elevation} feet</p>
    <p><span style="font-weight: bold">
    Effort:</span> ${selectedMountain.effort}</p>
    <img src="../images/${selectedMountain.img}" alt="${selectedMountain.name}">
    <p><span style="font-weight: bold">
    Description:</span> ${selectedMountain.desc}</p>
    <p>
    <span style="font-weight: bold">
    Coordinates:</span> Latitude ${selectedMountain.coords.lat}, Longitude ${selectedMountain.coords.lng}</p>
  `;
}
...
selectMountainEl.onchange = function () {
  ...
  const mountainInfoContent = createMountainInfoHTML(selectedMountain);

  while (mountainInfoDiv.firstChild) {
    mountainInfoDiv.removeChild(mountainInfoDiv.firstChild);
  }

  mountainInfoDiv.insertAdjacentHTML("beforeend", mountainInfoContent);
};
